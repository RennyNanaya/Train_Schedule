class JobBucketizer {
  constructor(data) {
    this.raw = data;
    this.sorted = this.sortJobs(data);
    this.buckets = this.organize(this.sorted);
  }
...
fetch("schedule.json")
  ...
  .filter(job => job.date) // Keep anything with a 'date' field, even if it's invalid
  .sort((a, b) => {
    const aDate = new Date(a.date);
    const bDate = new Date(b.date);
    return isNaN(aDate) - isNaN(bDate) || aDate - bDate;
  });
    organizeIntoBuckets(sortedJobs);
  })
  .catch(err => console.error(err))


function organizeIntoBuckets(jobs) {
  const buckets = {
    invalidDates: [],
    pastJobs: [],
    thisWeek: [],
  ...
  const lastSunday = getWeekStart(today, -1); // Previous week
  const thisSunday = getWeekStart(today, 0);
  const nextSunday = getWeekStart(today, 1);
  const weekAfterNextSunday = getWeekStart(today, 2);

  jobs.forEach(job => {
    const jobDate = new Date(job.date);
    if (isNaN(jobDate)) {
      buckets.invalidDates.push(job);
    } else if (jobDate < thisSunday) {
      buckets.pastJobs.push(job);
    } else if (jobDate >= thisSunday && jobDate < nextSunday) {
      buckets.thisWeek.push(job);
    } else if (jobDate >= nextSunday && jobDate < weekAfterNextSunday) {
      buckets.nextWeek.push(job);
    } else {
      buckets.futureJobs.push(job);
    }
  });

  console.log(buckets); // or pass them to your renderer
}
...
function getWeekStart(baseDate, offset = 0) {
  ...
  sunday.setDate(date.getDate() - day + (offset * 7));
